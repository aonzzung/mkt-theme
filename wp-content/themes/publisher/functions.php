...
function publisher_scripts() {
	...
	//Fitvids
	wp_enqueue_script( 'publisher-fitvids-js', get_template_directory_uri() . '/js/jquery.fitvids.js', array( 'jquery' ), '1.0.3', true );
...
function publisher_featured_image_url() {
	...
	if ( !is_object( $post ) ) {
		...
		wp_localize_script( 'publisher-custom-js', 'publisher_bg_js_vars', array(
				'bg_image' => get_option( 'publisher_customizer_bg_disable' )
			)
		);
	}
